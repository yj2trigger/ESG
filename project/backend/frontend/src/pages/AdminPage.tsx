...
function PowerGraph({
  machineId, token, startThreshold, stopThreshold, refreshKey,
}: {
  machineId: number; token: string; startThreshold: number; stopThreshold: number; refreshKey?: number
}) {
  ...
  useEffect(() => {
    if (refreshKey !== undefined && refreshKey > 0 && selectedDate === todayKST()) {
      fetchData(selectedDate)
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [refreshKey])
    ...
    <div style={styles.graphContainer}>
      ...
      <ResponsiveContainer width="100%" height={160}>
        <LineChart data={chartData} margin={{ top: 4, right: 8, left: 0, bottom: 4 }}>
          ...
          <Tooltip
            labelFormatter={(v) => fmtHHMMSS(Number(v))}
...
export default function AdminPage() {
  ...
  const [powerRefreshKeys, setPowerRefreshKeys] = useState<Record<number, number>>({})
  ...
  useWebSocket(token, (msg: WsMessage) => {
    ...
    if (msg.type === 'power_updated' && msg.machine_id !== undefined) {
      setPowerRefreshKeys(prev => ({ ...prev, [msg.machine_id!]: (prev[msg.machine_id!] ?? 0) + 1 }))
      reloadMachines()
    }
  })
              ...
              <div key={m.id}>
                ...
                {expandedGraphs.has(m.id) && (
                  <PowerGraph
                    ...
                    refreshKey={powerRefreshKeys[m.id] ?? 0}
                  />
                )}
              </div>
            ))}
